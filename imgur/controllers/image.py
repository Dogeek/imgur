...
class ImageController(Controller):
    def image(self, image_hash):
        data = self.get(f'/3/image/{image_hash}').json()['data']
    ...
    def upload(self, image=None, video=None, album=None, type=None,
        ...
        if (image is None and video is None) or (image is not None and video is not None):
            raise ImgurClientError('You need to provide either an image or a video to upload')
        ...
        if video is not None and mimetypes.guess_type(video)[0] not in accepted_types:
            raise ImgurClientError('Video mimetype is not accepted. Accepted types: {", ".join(t.split("/")[1] for t in accepted_types)}')
        ...
        data = self.post('/3/upload', data=payload).json()
        image = Image(self.session)
        image.set(**data['data'])
        return image

    def delete(self, hash):
        return super().delete(f'/3/image/{hash}').json()['data']
    ...
    def update(self, hash, title=None, description=None):
        ...
        return self.post(f'/3/image/{hash}', data=payload).json()['data']

    def favorite(self, hash):
        return self.post(f'/3/image/{hash}/favorite').json()['data']
